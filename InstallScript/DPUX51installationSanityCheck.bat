...
Set DB_info=192.168.10.73
rem 192.168.10.40 localhost
Set DBUSERPWD=sa/Si*****0

Set Branch_dir=LastBuild
...
Set Mail_subject=[GZteam: %Branch_Name% branch][Fast Test]Normalize5.1 Sanity Check Status Notification
...
Set A50_LogFile="%DP_Install_Path%\log\*"
Set N50_LogFile="%DP_Install_Path%\log\*"

Set DP_Install_Type=MSSQL_IISExpress
Set UX_Install_Type=MSSQL
Set DownloadfilePath=\\192.168.8.25\installer\BDNA\Normalize5.0\%Branch_dir%
...
:main_wrok_Item
rem Call all of the installation files
Call :VM_Information
...
call :Create_SalesDemo_TASK
call :Run_SalesDemo_TASK1
call :Run_SalesDemo_TASK2
...
if "%task_Status%" == "READY" Goto Check_SyncReady 
if "%task_Status%" == "RUNNING" Goto Check_Syncrunning 
if "%task_Status%" == "FINISH" Goto Check_SyncFinish 
if "%task_Status%" == "ERROR" Goto Check_SyncError 

if "%task_Status%" == "TIMEOUT" ( Goto Sync_Check_Timeout )
else (
echo [%time%] Sync running on the other exception, please take a look at the attach log file ...
Goto Mail_Notification_Failed )
...
"%DP_Install_Path%\bin\NormalizeCMD.exe" -CREATEPROCESS /PROCESS_NAME=HPUD /PROCESS_TYPE=NORMALIZE /SOURCE_TYPE="IT Discovery Tool,31" /DATASOURCE_TYPE=ZIP /NETWORK_PATH="\\192.168.8.25\app\BDNA\Normalize\SalesDemo\HPUD.zip" /NETWORK_USERNAME=bdnacn\sunkj  /NETWORK_PASSWORD=bdnacn

Echo Combine to IT-Mashup Task: DB:CM_XYZ
"%DP_Install_Path%\bin\NormalizeCMD.exe" -ADDDATASOURCE /COMBINE_TO=10000 /PROCESS_TYPE=NORMALIZE /SOURCE_TYPE="IT Discovery Tool,3" /DATASOURCE_TYPE=MSSQLSERVER /SERVER_NAME=192.168.8.8 /DATABASE=CM_XYZ /USERNAME=sa /PASSWORD=bdna202 /CONFIG="sccm.extractor.config"

Echo Combine to IT-Mashup Task: Mashup USer: HR
"%DP_Install_Path%\bin\NormalizeCMD.exe" -ADDDATASOURCE /COMBINE_TO=10000 /PROCESS_TYPE=DATA_MASHUP /SOURCE_TYPE="Data Mashup,34" /DATASOURCE_TYPE=FILE /NETWORK_PATH="\\192.168.8.25\app\BDNA\Normalize\SalesDemo\HR.csv" /FILE_OBJECT_TYPE=User /NETWORK_USERNAME=bdnacn\sunkj  /NETWORK_PASSWORD=bdnacn

Echo Create PO Task
"%DP_Install_Path%\bin\NormalizeCMD.exe" -CREATEPROCESS /PROCESS_TYPE=PURCHASE_ORDER /SOURCE_TYPE="Purchase Order,34" /DATASOURCE_TYPE=FILE /NETWORK_PATH="\\192.168.8.25\app\BDNA\Normalize\SalesDemo\CDW_DEMO.csv" /NETWORK_USERNAME=BDNACN\sunkj  /NETWORK_PASSWORD=bdnacn

Echo Create MD-IT-task
"%DP_Install_Path%\bin\NormalizeCMD.exe" -CREATEPROCESS /PROCESS_NAME="MD" /PROCESS_TYPE=NORMALIZE /SOURCE_TYPE="IT Discovery Tool,31" /DATASOURCE_TYPE=ZIP /NETWORK_PATH="\\192.168.8.25\app\BDNA\Normalize\SalesDemo\MD_Adventist_Health_System_SCCM_150722110427.zip" /NETWORK_USERNAME=bdnacn\sunkj  /NETWORK_PASSWORD=bdnacn

Echo Create MD-PO-task
"%DP_Install_Path%\bin\NormalizeCMD.exe" -CREATEPROCESS /PROCESS_TYPE=PURCHASE_ORDER /SOURCE_TYPE="Purchase Order,34" /DATASOURCE_TYPE=FILE /NETWORK_PATH="\\192.168.8.25\app\BDNA\Normalize\SalesDemo\PO_MD_SAMPLE_012015.csv" /NETWORK_USERNAME=BDNACN\sunkj  /NETWORK_PASSWORD=bdnacn
...
"%DP_Install_Path%\bin\NormalizeCMD.exe" -runprocess /process_id=10000
...
echo [%time%] "%DP_Install_Path%\bin\NormalizeCMD.exe" -CHECKPROCESSSTATUS /PROCESS_ID=10000 
:Re_check_TASK1Status
"%DP_Install_Path%\bin\NormalizeCMD.exe" -CHECKPROCESSSTATUS /PROCESS_ID=10000 >JOB_Status.txt
for %%2 in (JOB_Status.txt) do for /f "tokens=1" %%J in (%%2) do (
Set Task_Status=%%J
)
...
"%DP_Install_Path%\bin\NormalizeCMD.exe" -runprocess /process_id=10003
ping /n %OTintS% 127.1>nul
...
echo [%time%] "%DP_Install_Path%\bin\NormalizeCMD.exe" -CHECKPROCESSSTATUS /PROCESS_ID=10003
ping /n %OTint% 127.1>nul

:Re_check_TASK2Status
"%DP_Install_Path%\bin\NormalizeCMD.exe" -CHECKPROCESSSTATUS /PROCESS_ID=10003 >JOB_Status.txt
for %%2 in (JOB_Status.txt) do for /f "tokens=1" %%J in (%%2) do (
Set Task_Status=%%J
)
...
"%DP_Install_Path%\bin\NormalizeCMD.exe" -runprocess /process_id=10004
"%DP_Install_Path%\bin\NormalizeCMD.exe" -runprocess /process_id=10005
"%DP_Install_Path%\bin\NormalizeCMD.exe" -runprocess /process_id=10006
...
copy /Y "%A50_LogFile%" "%A50_LogFile%.%File_DataTime%" >zip_log.txt
copy /Y "%N50_LogFile%" "%N50_LogFile%.%File_DataTime%" >zip_log.txt
